...
* Add `getQueueDiagnostics(id)` with payload-free queue lifecycle, pending, and unresolved-active counts.
* Replacing the `UniversalBle` platform instance now clears all queue generations so fault state cannot leak across instances.
